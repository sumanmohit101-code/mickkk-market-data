...
const CANDLE_DB_NAME = 'MICKKK_CHART_LOCAL_CACHE_V2';
...
// CPR: Default is showLabels = false (ONLY LINES VISIBLE, NO OVERLAPPING TEXT)
...
function sanitizeCandles(candles) {
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    let volume = Number(c.volume || 0);

    // Anti-Spike Guard: If high is 3.5x open/close due to data corruption or split glitch, normalize it
    const base = (open + close) / 2;
    if (base > 0) {
      if (high > base * 3.5) high = Math.max(open, close) * 1.05;
      if (low < base * 0.25) low = Math.min(open, close) * 0.95;
    }
...
function formatXAxisDate(time) {
  ...
  if (time && typeof time === 'object' && !Array.isArray(time)) {
    if (Number.isFinite(Number(time.year)) && Number.isFinite(Number(time.month)) && Number.isFinite(Number(time.day))) {
      year = Number(time.year); month = Number(time.month); day = Number(time.day);
    }
...
async function openMyAlertsModal() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.alerts) { renderMyAlerts(data.alerts); return; }
  } catch (e){}
...
async function deleteMyAlert(id, btn) {
  ...
  try { await fetch(`${CHARTS_API_URL}?action=deleteAlert&id=${encodeURIComponent(id)}`); } catch (e){}
...
function ensureSeriesType(panel) {
  ...
  if (currentChartType === 'bar') {
    panel.candleSeries = panel.priceChart.addBarSeries({ upColor: '#26a69a', downColor: '#ef5350', thinBars: false });
...
function setPriceScaleMode(mode, btn) {
  ...
  panelsArray.forEach(p => {
    if (p.priceChart) p.priceChart.priceScale('right').applyOptions({ mode: scaleMode });
  });
}
...
function setInterval_(interval, btn){
  ...
  if (btn) btn.classList.add('active');
  panelsArray.forEach(p => { p.interval = interval; });
  if (currentSymbol) rebuildAllPanels();
}
...
function rebuildGridSystem() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Note: Removed the dummy broken 28px pane-axis. The bottom-most active pane is the native date axis.
    chartContainer.innerHTML = `
      ...
      <div class="unified-crosshair-v" id="unified-crosshair-${i}">
        <div class="unified-crosshair-date" id="unified-crosshair-date-${i}"></div>
      </div>
    `;
...
/* ============ DYNAMIC BOTTOM DATE AXIS VISIBILITY ============ */
function updateTimeScaleVisibility(panel) {
  ...
  // Identify the physical bottom-most pane to render the clean, crisp date axis
  let bottomChart = panel.priceChart;
  if (showRs) bottomChart = panel.rsChart;
  else if (showRsi) bottomChart = panel.rsiChart;

  [panel.priceChart, panel.rsiChart, panel.rsChart].forEach(c => {
    if (!c) return;
    const isBottom = (c === bottomChart);
    c.timeScale().applyOptions({
      visible: isBottom,
      height: isBottom ? 26 : 0,
      timeVisible: false,
      secondsVisible: false,
      ticksVisible: isBottom,
      rightOffset: 0,
...
function createPanelChartInstance(index, tf) {
  ...
  const baseChartOpts = {
    ...
    timeScale: {
      visible: false, timeVisible: false, secondsVisible: false, ticksVisible: false,
      uniformDistribution: true, rightOffset: 0, tickMarkFormatter: (time) => formatXAxisDate(time),
      borderColor: isDark ? '#1e2d3d' : '#e0e3eb'
    },
  ...
  const priceC = LightweightCharts.createChart(priceContainer, {
    ...baseChartOpts,
    rightPriceScale: { borderColor: isDark ? '#1e2d3d' : '#e0e3eb', mode: scaleMode, minimumWidth: 68 }
  });
  ...
  const rsiC = LightweightCharts.createChart(rsiContainer, { ...baseChartOpts });

  const rsContainer = document.getElementById(`pane-rs-${index}`);
  const rsC = LightweightCharts.createChart(rsContainer, { ...baseChartOpts });

  const canvas = document.getElementById(`panel-canvas-${index}`);

  const instObj = {
    index: index, priceChart: priceC, rsiChart: rsiC, rsChart: rsC,
    chart: priceC, candleSeries: null, seriesType: null, volumeSeries: volS, interval: tf,
    symbol: currentSymbol, symbolName: currentName, instrumentType: currentInstrumentType || 'stock',
...
function initPaneSplitterDrag(index) {
  ...
  window.addEventListener('mouseup', () => { isDragging1 = false; splitter1.classList.remove('dragging'); });
...
function navigateStockKeyboard(direction) {
  ...
  if (target) loadSymbol(target.symbol, target.name, -1, currentInstrumentType);
}
...
function handleGlobalKeyDown(e) {
  ...
  if (e.key === 'ArrowUp') { e.preventDefault(); navigateStockKeyboard(-1); return; }
  else if (e.key === 'ArrowDown') { e.preventDefault(); navigateStockKeyboard(1); return; }

  if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); openSearchModal(-1); return; }
...
function renderWatchlists(){
  ...
  (wl.sections || []).forEach(section => {
    ...
    head.querySelector('.watch-section-menu').onclick = (e) => { e.stopPropagation(); sectionMenuUI(section.name); };
    ...
    if (!collapsed) {
      ...
      if (!(section.items || []).length) items.innerHTML = '<div class="watch-empty" style="padding:8px 4px;text-align:left;">Empty section</div>';
...
async function addStockUI(){
  ...
  if (!allSymbols || !allSymbols.length) { alert('Stock list could not be loaded.'); return; }
  ...
  const selectHtml = allSymbols.map((x,i)=>`<option value="${i}">${escapeWatchText(String(x.symbol))} — ${escapeWatchText(String(x.name||''))}</option>`).join('');
  const sectionHtml = sections.map(s=>`<option value="${escapeWatchText(String(s.name))}">${escapeWatchText(String(s.name))}</option>`).join('');
  ...
  picker.querySelector('#watch-stock-add').onclick=async()=>{
    ...
    await mutateWatchlist('addSymbol',{
      watchlist:activeWatchlistName, section,
...
function filterSearchModal(){
  ...
  let list = source;
  if (q) list = list.filter(s => (s.name && s.name.toLowerCase().includes(q)) || (s.symbol && s.symbol.toLowerCase().includes(q)));
...
function openCandleCacheDB(){
  ...
  candleDbPromise = new Promise((resolve) => {
    try {
      ...
      req.onupgradeneeded = () => {
        const db = req.result;
        if (!db.objectStoreNames.contains(CANDLE_DB_STORE)) db.createObjectStore(CANDLE_DB_STORE, { keyPath: 'symbol' });
      };
...
async function readCandleCache(symbol, type){
  try {
    ...
    if (result && Array.isArray(result.candles)) {
      result.candles = sanitizeCandles(result.candles);
    }
    return result;
  } catch(e) { return null; }
}
...
async function writeCandleCache(symbol, type, candles){
  try {
    ...
    await new Promise(resolve => {
      ...
      tx.objectStore(CANDLE_DB_STORE).put({ symbol: candleCacheKey(symbol, type), candles, savedAt: Date.now() });
...
async function fetchFreshCandlesFromStatic(symbol, type='stock'){
  ...
  return sanitizeCandles(candles).sort((a,b)=>a.time < b.time ? -1 : 1);
}
...
async function fetchFreshCandlesFromServer(symbol){
  ...
  const data = await res.json();
  if (data.error) throw new Error(data.error);
  return sanitizeCandles(data.candles || []).sort((a,b)=>a.time < b.time ? -1 : 1);
}
  ...
  try {
    ...
    if (cached) {
      ...
      if (Array.isArray(parsed) && parsed.length) {
        const clean = sanitizeCandles(parsed);
        symbolCandleCache[key] = clean;
        symbolCandleCacheMeta[key] = Date.now();
        writeCandleCache(symbol,type,clean);
        return clean;
      }
    }
  } catch(e) {}
...
async function rebuildPanelChart(panel) {
  ...
  try {
    ...
    else panel.volumeSeries.setData([]);
...
/* ============ CPR ENGINE (LINES ONLY BY DEFAULT) ============ */
...
function drawCPRLine(ctx,x1,x2,y,label,value,dash=false){
  ...
  ctx.save(); 
  ctx.strokeStyle='rgba(255,171,0,.88)'; 
  ctx.lineWidth=1; 
  ...
  // ONLY DRAW TEXT LABELS IF USER ACTIVELY ENABLES THEM (DEFAULT IS HIDDEN)
  if (cprConfig.showLabels) {
    ctx.fillStyle='rgba(255,171,0,.95)';
...
function drawCPROverlay(panel,data){
  ...
  modes.forEach(mode=>{
    ...
    vals.forEach((item,idx)=>{
      ...
      drawCPRLine(ctx,x1,x2,y,`${mode==='W'?'W':'M'} ${item[0]}`,item[1],idx===1);
    });
  });
}
  ...
  try {
    ...
    statsWidget.innerHTML = `
      <div class="ppv-table-row">
        <div class="ppv-cell ${udRatio >= 1 ? 'bg-green' : 'bg-red'}">U/D Volume Ratio: ${udRatio.toFixed(2)}</div>
        <div class="ppv-cell bg-neutral">Avg Volume (Cr): ${avgVolCr.toFixed(1)}</div>
        <div class="ppv-cell ${relVol >= 0 ? 'bg-green' : 'bg-red'}">Rel Volume: ${relVol >= 0 ? '+' : ''}${relVol.toFixed(2)}%</div>
      </div>
    `;
  } catch(e){}
...
function renderRSIPane(data, panel) {
  ...
  if (rsiConfig.enabled) {
    ...
    for (let i = 0; i < Math.min(period, data.length); i++) rsiData.push({ time: data[i].time });
    ...
    if (!panel.rsiSeries) {
      panel.rsiSeries = panel.rsiChart.addLineSeries({ color: rsiConfig.color, lineWidth: rsiConfig.width, priceLineVisible: false, lastValueVisible: true });
    } else {
    ...
    if (rsiMAConfig.enabled && rsiValuesArr.length) {
      ...
      if (!panel.rsiMASeries) {
        panel.rsiMASeries = panel.rsiChart.addLineSeries({ color: rsiMAConfig.color, lineWidth: rsiMAConfig.width, priceLineVisible: false, lastValueVisible: false });
      } else {
    ...
    if (rsiConfig.showThresholds && panel.rsiSeries) {
      try {
        const uLine = panel.rsiSeries.createPriceLine({ price: rsiConfig.upperVal || 60, color: '#ff3d5a88', lineWidth: 1, lineStyle: LightweightCharts.LineStyle.Dashed, axisLabelVisible: false });
        const lLine = panel.rsiSeries.createPriceLine({ price: rsiConfig.lowerVal || 40, color: '#00e67688', lineWidth: 1, lineStyle: LightweightCharts.LineStyle.Dashed, axisLabelVisible: false });
...
async function renderRelativeStrengthPane(data,panel){
  ...
  try{
    ...
    if(!stockData.length || !benchData.length) return;

    const benchMap=new Map(benchData.map(c=>[String(c.time),Number(c.close)]));
    const common=[];
    for(const c of stockData){
      const b=benchMap.get(String(c.time));
      const close=Number(c.close);
      if(Number.isFinite(close) && close>0 && Number.isFinite(b) && b>0) common.push({time:c.time,ratio:close/b});
    }
    if(!common.length) return;

    const base=common[0].ratio || 1;
    const rsData=common.map(x=>({time:x.time,value:+(x.ratio/base*100).toFixed(4)}));
    ...
    const maData=[]; let sum=0;
    for(let i=0;i<rsData.length;i++){
      sum+=rsData[i].value;
      if(i>=maLen)sum-=rsData[i-maLen].value;
      if(i>=maLen-1)maData.push({time:rsData[i].time,value:+(sum/maLen).toFixed(4)});
    }
    ...
    try{
      ...
      if(range) panel.rsChart.timeScale().setVisibleLogicalRange(range);
    }catch(e){}
...
function applyRangeToPanel(panel, data) {
  ...
  if (currentRange === 'ALL') {
    [panel.priceChart, panel.rsiChart, panel.rsChart].forEach(c => { try { c.timeScale().fitContent(); } catch(e){} });
    return;
  }
...
/* ============ LIVE POLLING & FAST UPDATE ENGINE ============ */
...
function applyLivePrice(price){
  ...
  // ANTI-SPIKE GUARD: If price jumps > 25% in 1 tick compared to previous close, reject as bad API data
  if (prevClose > 0 && Math.abs(p - prevClose) / prevClose > 0.25) {
    console.warn(`[Spike Guard] Rejecting abnormal live price: ₹${p} (Previous: ₹${prevClose})`);
    return;
  }
  ...
  // Fast Update: Updates existing candles without full reload or losing zoom/pan
  panelsArray.forEach(panel => {
    if (panel.symbol === currentSymbol && panel.candleSeries) {
      const data = aggregate(primary.rawDailyCandles, panel.interval);
      if (!data.length) return;
      const updatedLast = data[data.length - 1];
      try {
        if (currentChartType === 'line') {
          panel.candleSeries.update({ time: updatedLast.time, value: updatedLast.close });
        } else {
