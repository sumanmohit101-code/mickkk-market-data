  ...
  for (let i = 0; i < count; i++) {
    ...
    topBar.innerHTML = `
      <button class="panel-stock-btn" onclick="openSearchModal(${i})">
        <span style="font-size:9px;color:var(--accent);">${currentInstrumentType==='index'?'IDX':'EQ'}</span> 🔍 <span>${currentSymbol}</span> ▼
      </button>
...
/* DEDICATED SOLE BOTTOM DATE AXIS BAR ENGINE */
...
function createPanelChartInstance(index, tf) {
  ...
  const axisC = LightweightCharts.createChart(document.getElementById(`pane-axis-${index}`), {
    ...
    timeScale: { visible: true, timeVisible: false, secondsVisible: false, ticksVisible: true, borderVisible: true, borderColor: isDark ? '#1e2d3d' : '#e0e3eb', tickMarkFormatter: (t) => formatXAxisDate(t), uniformDistribution: true, rightOffset: 0 },
  ...
  const instObj = {
    index, priceChart: priceC, rsiChart: rsiC, rsChart: rsC, axisChart: axisC, axisSeries,
    ...
    rsiSeries: null, rsiMASeries: null, rsiThresholdLines: [], rsSeries: null, rsMASeries: null, rsValueByTime: []
  };
  ...
  const syncPanes = (source) => {
    source.timeScale().subscribeVisibleLogicalRangeChange(range => {
      ...
      isSyncing = true;
      [priceC, rsiC, rsC, axisC].forEach(c => { if (c && c !== source) try { c.timeScale().setVisibleLogicalRange(range); } catch(e){} });
      isSyncing = false;
...
function setupUnifiedCrosshairEngine(instObj) {
  ...
  container.addEventListener('mousemove', (e) => {
    ...
    if (!time) {
      ...
      if (instObj.index === 0) { const b = document.getElementById('ohlc-box'); if (b) b.style.display = 'none'; }
      return;
    }
...
function updateOHLCBox(bar) {
  ...
  const chg = c - o, pct = o ? (chg / o * 100) : 0, cls = chg >= 0 ? 'lg' : 'lr';
  box.style.display = 'flex';
  box.innerHTML = `<b style="color:var(--text);">${currentSymbol}</b><span style="color:var(--border);">|</span><span>O <b>${o.toFixed(2)}</b></span><span>H <b>${h.toFixed(2)}</b></span><span>L <b>${l.toFixed(2)}</b></span><span>C <b>${c.toFixed(2)}</b></span><span class="${cls}">${chg>=0?'+':''}${chg.toFixed(2)} (${pct>=0?'+':''}${pct.toFixed(2)}%)</span>${bar.volume?'<span style="color:var(--border);">|</span> Vol <b>'+fmtVol(bar.volume)+'</b>':''}`;
}
...
async function loadSymbol(symbol, name, targetIdx, instrumentType='stock'){
  ...
  if (targetIdx === -1 || targetIdx === undefined) {
    ...
    const l = document.getElementById('top-stock-btn-label'); if (l) l.innerText = `${type==='index'?'📊':'🔍'} ${symbol}`;
  }
  document.getElementById('empty-state').style.display = 'none';
  document.getElementById('loading').style.display = 'flex';
  try {
    panelsArray.forEach(p => { p.symbol = symbol; p.symbolName = name; p.instrumentType = type; });
    await rebuildAllPanels();
    updateHeader();
    startLivePoll();
  } catch(e){ showChartError(e.message); }
  finally { document.getElementById('loading').style.display = 'none'; closeSearchModal(); }
}

async function rebuildAllPanels() {
  await Promise.all(panelsArray.map(p => rebuildPanelChart(p)));
  const primary = panelsArray[0];
  if (primary && primary.rawDailyCandles.length) renderCombinedInfoCard(aggregate(primary.rawDailyCandles, 'D'));
}
...
async function rebuildPanelChart(panel) {
  ...
  try {
    ...
    if (panel.axisSeries) panel.axisSeries.setData(data.map(d => ({ time: d.time, value: 0 })));
    ensureSeriesType(panel);

    if (currentChartType === 'line') panel.candleSeries.setData(data.map(d => ({ time: d.time, value: d.close })));
    else panel.candleSeries.setData(data);

    if (volVisible) panel.volumeSeries.setData(data.map(c => ({ time: c.time, value: c.volume||0, color: c.close>=c.open?'#00e67688':'#ff3d5a88' })));
...
function ensureSeriesType(panel) {
  ...
  if (currentChartType === 'bar') panel.candleSeries = panel.priceChart.addBarSeries({ upColor: '#26a69a', downColor: '#ef5350' });
  else if (currentChartType === 'line') panel.candleSeries = panel.priceChart.addLineSeries({ color: '#00d4ff', lineWidth: 2 });
  else panel.candleSeries = panel.priceChart.addCandlestickSeries({ upColor: '#26a69a', downColor: '#ef5350', borderUpColor: '#26a69a', borderDownColor: '#ef5350', wickUpColor: '#26a69a', wickDownColor: '#ef5350' });
}

function updateHeader(){
  const p = panelsArray[0]; if (!p || !p.rawDailyCandles.length) return;
...
function applyRangeToPanel(panel, data) {
  ...
  if (currentRange === 'ALL') { [panel.priceChart, panel.rsiChart, panel.rsChart, panel.axisChart].forEach(c => c && c.timeScale().fitContent()); return; }
  ...
  [panel.priceChart, panel.rsiChart, panel.rsChart, panel.axisChart].forEach(c => { if(c) try { c.timeScale().setVisibleLogicalRange(range); } catch(e){} });
}

function setRange(r, btn){ currentRange = r; btn.parentElement.querySelectorAll('.tbtn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); panelsArray.forEach(p => applyRangeToPanel(p, aggregate(p.rawDailyCandles, p.interval))); }
function setInterval_(i, btn){ currentInterval = i; btn.parentElement.querySelectorAll('.tbtn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); panelsArray.forEach(p => p.interval = i); rebuildAllPanels(); }
function setChartType(t, btn){ currentChartType = t; btn.parentElement.querySelectorAll('.tbtn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); rebuildAllPanels(); }
function setPriceScaleMode(m, btn){ currentScaleMode = m; btn.parentElement.querySelectorAll('.tbtn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); rebuildGridSystem(); }
function toggleVolume(btn){ volVisible = !volVisible; btn.classList.toggle('active'); rebuildAllPanels(); }
function toggleTheme(){ currentTheme = currentTheme === 'dark' ? 'light' : 'dark'; document.body.classList.toggle('light-theme', currentTheme === 'light'); rebuildGridSystem(); }
function toggleDrawingToolbar(){ document.getElementById('drawing-toolbar')?.classList.toggle('collapsed'); document.querySelector('.main')?.classList.toggle('tools-collapsed'); }
function setTool(t, btn){ activeTool = t; document.querySelectorAll('.draw-btn').forEach(b => b.classList.remove('active')); if (btn) btn.classList.add('active'); }
function zoomChart(m){ panelsArray.forEach(p => { const ts = p.priceChart.timeScale(), r = ts.getVisibleLogicalRange(); if (r) { const half = Math.max(3, (r.to - r.from) * m / 2), mid = (r.from + r.to) / 2; ts.setVisibleLogicalRange({ from: mid - half, to: mid + half }); } }); }
function resetChartView(){ panelsArray.forEach(p => applyRangeToPanel(p, aggregate(p.rawDailyCandles, p.interval))); }
function toggleFullScreen(){ if (!document.fullscreenElement) document.documentElement.requestFullscreen(); else document.exitFullscreen(); }
function captureChartScreenshot(){ if (panelsArray[0]) { const c = panelsArray[0].priceChart.takeScreenshot(); const a = document.createElement('a'); a.download = `${currentSymbol}_Chart.png`; a.href = c.toDataURL(); a.click(); } }

function openSearchModal(idx){ searchTargetPanelIndex = idx; document.getElementById('search-modal').classList.add('open'); filterSearchModal(); }
function closeSearchModal(){ document.getElementById('search-modal').classList.remove('open'); }
function setSearchMode(m){ searchMode = m; document.getElementById('tab-stocks').classList.toggle('active', m==='stock'); document.getElementById('tab-indices').classList.toggle('active', m==='index'); filterSearchModal(); }
...
function toggleWatchlistPanel(force){ const p = document.getElementById('watchlist-panel'); p.classList.toggle('open', force !== undefined ? force : !p.classList.contains('open')); }
...
function initPaneSplitterDrag(index) {
  ...
  let drag = false; sp.onmousedown = () => drag = true; window.onmouseup = () => drag = false;
  window.addEventListener('mousemove', e => { if (!drag) return; panePrice.style.flex = `0 0 ${Math.max(100, e.clientY - container.getBoundingClientRect().top)}px`; });
}
...
function toggleIndicator(k){ activeIndicators[k] = !activeIndicators[k]; rebuildAllPanels(); }
...
function showChartError(msg){
  ...
  es.style.display = 'flex'; es.innerHTML = `<div style="font-size:32px;">⚠️</div><div>${msg}</div>`;
}
...
function clearDrawings(){ userDrawings = []; rebuildAllPanels(); }
function handleChartMouseDown(e) {}
function handleChartMouseMove(e) {}

/* INITIALIZE ENGINE */
rebuildGridSystem();
loadSymbol('ABCAPITAL', 'Aditya Birla Capital Limited', -1);
